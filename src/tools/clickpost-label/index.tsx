...
export default function ClickpostLabelTool() {
  ...
  const generate = async (action: 'download' | 'print') => {
    ...
    setBusy(true)
    try {
      const bytes = await composeSheet(slots.map((slot) => slot?.pngBytes ?? null))
      if (action === 'download') {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm leading-relaxed text-gray-500">
        ...
        など)の好きな面に配置し直したPDFを作ります。ファイルはブラウザ内でのみ処理され、外部には送信されません。
      </p>
        ...
        <div>
          ...
          <section className="mt-8 rounded-md bg-white p-4 text-sm text-gray-600 shadow-sm">
            ...
            <ol className="mt-2 list-decimal space-y-1 pl-5">
              ...
              <li>「PDFをダウンロード」または「印刷」で出力し、A4・4面シール用紙に倍率100%で印刷する</li>
            </ol>
          </section>
        </div>
